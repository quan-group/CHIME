...
FUN.run_outcomes <- function(x, year) {
  ...
  x[year, "retinopathy_all"] <- ifelse(x[year, "history_retinopathy_all"] == 1, 1, FUN.outcomes(all_models[["retinopathy_all_model"]][[1]][[1]], x[year,], year))
...
FUN.run_outcomes2 <- function(x, year) {
  ...
  x[year, "retinopathy_all"] <- ifelse(x[year, "history_retinopathy_all"] == 1, 1, FUN.outcomes2(all_models[["retinopathy_all_model"]][[1]][[1]], x[year-1,], x[year,], year))
...
FUN.run_additional_year <- function(x, year){
  if (x[year-1, "death"] == 0 & !is.na(x[year-1, "death"])) {
    ...
    x[year, "history_retinopathy_all"] <- max(x[year-1, "history_retinopathy_all"], x[year-1, "retinopathy_all"])
